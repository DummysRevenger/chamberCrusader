...
public class gridmaker3 : MonoBehaviour
{
    ...
    {
        float smallest = gridmaker3.gridWeights[0, 0];
        ...
        for (int i = 0; i < numRows; i++)
        {
            for (int j = 0; j < numCols; j++)
            {
                ...
                if (gridmaker3.gridWeights[i, j] < smallest)
                {
                    smallest = gridmaker3.gridWeights[i, j]; // Update the smallest value
                    smallesti = i;
                    smallestj = j;
                }
    ...
    public GameObject findSmallestHeuristic()
    {
        ...
        calculateDistanceOfGrid(cellToGoToNext, player);
    ...
    public static void calculateDistanceOfGrid(GameObject ooi, GameObject player)
    {
        for (int row = 0; row < numRows; row++)
        {
            for (int col = 0; col < numCols; col++)
            {
                ...
                if (cell != null && cell != ooi && gridmaker3.S.grid[row, col] != 1)
                {
                ...
                else if (cell != null && gridmaker3.S.grid[row, col] != 1)
                {
    ...
    {
        if (!gridmaker3.S.cellReached)
        {
            gridmaker3.S.target = gridmaker3.S.findSmallestHeuristic();
        }

        gridmaker3.S.target = gridmaker3.S.findSmallestHeuristic();


        Vector3 direction = (gridmaker3.S.target.transform.position - otm.transform.position).normalized;
        otm.transform.position += direction * speed * Time.deltaTime;

        float distanceToTarget = CalculateEuclideanDistance(otm.transform.position.x, otm.transform.position.y, gridmaker3.S.target.transform.position.x, gridmaker3.S.target.transform.position.y);

        if (distanceToTarget < 0.1f)
        {


            gridmaker3.S.cellReached = true;

            gridmaker3.S.returnClosestTriangle();
